...
const OrderSummary = ({ totalPrice, items }: OrderSummaryProps) => {
	...
	const handleCouponCode = async (event: React.FormEvent<HTMLFormElement>) => {
		event.preventDefault();
		// Coupon logic goes here...
	};

	const handlePlaceOrder = async (e: React.MouseEvent<HTMLButtonElement> | React.FormEvent) => {
		e.preventDefault();
		router.push('/orders');
	};
						...
						{addressList.length > 0 && (
							<select
								...
								onChange={(e) => {
									...
									setSelectedAddress(
										value !== '' ? addressList[Number(value)] : null,
									);
								}}
							>
			...
			<div className="pb-4 border-b border-slate-200">
				...
				{!coupon ? (
					...
					<div className="w-full flex items-center justify-center gap-2 text-xs mt-2">
						<p>
							Code:{' '}
							<span className="font-semibold ml-1">{coupon.code?.toUpperCase()}</span>
						</p>
